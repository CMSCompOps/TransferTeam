...
OUT=/afs/cern.ch/user/m/mtaze/work/public/OUTPUT/RegularCheckResults/replica_check/$current_week
mkdir -p $OUT
...
out_ds_without_custodial_location_final_list=$OUT/mail_dataset_without_custodial_location.out
out_ds_without_replica=$OUT/mail_dataset_without_replica.out

out_penguings_bunnies=$OUT/PenguinsBunnies.out
out_test_backfill=$OUT/mail_TestBackfill.out
out_t0test=$OUT/mail_T0Test.out
tmp=$OUT/tmp.out
...
[ -s $out_ds_without_replica ] && /bin/mail -s 'datasets without replica' cms-comp-ops-transfer-team@cern.ch < $out_ds_without_replica
...
[ -s $out_test_backfill ] && /bin/mail -s 'global Test and Backfill datasets' dmason@fnal.gov,andrew.lahiff@stfc.ac.uk,vincenzo.spinoso@cern.ch,ajit@hep.wisc.edu,jen_a@fnal.gov,jbadillo@cern.ch,Dirk.Hufnagel@cern.ch,gutsche@fnal.gov,amlevin@mit.edu,alan.malta@cern.ch,cms-comp-ops-transfer-team@cern.ch < $out_test_backfill
...
[ -s $out_t0test ] && /bin/mail -s 'T0 test datasets' dmason@fnal.gov,luis89@fnal.gov,Dirk.Hufnagel@cern.ch,gutsche@fnal.gov,cms-comp-ops-transfer-team@cern.ch < $out_t0test
