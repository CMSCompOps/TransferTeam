...
OUT=/afs/cern.ch/user/m/mtaze/work/public/OUTPUT/RegularCheckResults/production_check/$current_week
mkdir -p $OUT

# set files
out_prod_datasets=$OUT/mail_production_datasets.list
out_replica_location=$OUT/dataset_location.out
out_ds_without_replica=$OUT/mail_production_dataset_without_replica.out
out_test_dataset=$OUT/mail_test_dataset.out
tmp=$OUT/tmp.out
...
[ -s $out_ds_without_replica ] && /bin/mail -s 'production datasets without replica' dmason@fnal.gov,andrew.lahiff@stfc.ac.uk,vincenzo.spinoso@ba.infn.it,ajit@hep.wisc.edu,jbadillo@cern.ch,gutsche@fnal.gov,Dirk.Hufnagel@cern.ch,amlevin@mit.edu,cms-comp-ops-transfer-team@cern.ch < $out_ds_without_replica
